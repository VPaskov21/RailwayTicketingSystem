...
public class SearchController implements Serializable {
    ...
    public String findRoutes(Model model, @ModelAttribute(name = "departureStation") String departureStation,
        ...
        SearchData searchData = new SearchData(stations);
        ...
        List<RouteList> routes = Collections.emptyList();

        if(departureStation != arrivalStation){
            routes = routeService.findRoutesBetweenStations(departureStation, arrivalStation, departureDate);
            if(routes.size()==0){
                areAnyResults = false;
            }else{
                areAnyResults = true;
